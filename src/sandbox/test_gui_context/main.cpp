...
struct App : public ws::App {
  ...
  std::string experiment_date{ "20250505" };
  ...
  float maximalforce{ 1000.0f }; // 800 // 850 // in the unit of gram
  ...
  float lever1_forceperc{ 0.35f }; // % percentage of max force level (800 grams)
  float lever2_forceperc{ 0.35f }; // % percentage of max force level (800 grams)
  //
...
json get_ni_json_data(const ws::led::LEDSync* sync, ws::TimePoint session_t0) {
  ...
  const auto all_trigger_ts = ws::ni::read_trigger_time_points(); // vector<vector<TriggerTimePoint>>
...
void render_gui(App& app) {
  ...
  if (1) {
    ...
    if (ImGui::InputFloat("lever1 force percantage", lever1_forceperc_gui, 0.0f, 0.0f, "%0.2f", enter_flag)) {
      app.lever1_forceperc = lever1_forceperc_gui[0];
    };

    float lever2_forceperc_gui[1]{ app.lever2_forceperc };
    if (ImGui::InputFloat("lever2 force percantage", lever2_forceperc_gui, 0.0f, 0.0f, "%0.2f", enter_flag)) {
...
bool initialize_ni() {
  ...
  ws::ni::ChannelDescriptor ai_input_descs[2];
